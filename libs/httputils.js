...
function _createTypeEffectivenessWrapper(typeEffectivenesses) {
  let typesHTML = `
		${Object.keys(typeEffectivenesses)
      .map((effectiveness) => {
        ...
        return `
	      <div class="pokemon-${effectiveness} tooltip">
	          ...
	          ${typeEffectivenesses[effectiveness]
              .map(
                (type, counter) => `
	              ...
	              ${
                  (counter + 1) % 3 === 0 ||
                  counter + 1 === typeEffectivenesses[effectiveness].length
                    ? `</div>`
                    : ""
                }
	          `,
...
class HttpUtils {
  ...
  static createCardsDiv(divId) {
    ...
    document
      .getElementById(buttonsObj.idUp)
      .addEventListener("click", _changePage);
    document
      .getElementById(buttonsObj.idDown)
      .addEventListener("click", _changePage);
